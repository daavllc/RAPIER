...
namespace DAGGer {
	...
	struct EntityScriptClass
	{
		...
		{
			Constructor = GetMethod(s_CoreAssemblyImage, "DAGGer.Entity:.ctor(ulong)");
			OnCreateMethod = GetMethod(image, FullName + ":OnCreate()");
			OnUpdateMethod = GetMethod(image, FullName + ":OnUpdate(single)");
			OnPhysicsUpdateMethod = GetMethod(image, FullName + ":OnPhysicsUpdate(single)");

			// Physics (Entity class)
			OnCollisionBeginMethod = GetMethod(s_CoreAssemblyImage, "DAGGer.Entity:OnCollisionBegin(single)");
			OnCollisionEndMethod = GetMethod(s_CoreAssemblyImage, "DAGGer.Entity:OnCollisionEnd(single)");
			OnTriggerBeginMethod = GetMethod(s_CoreAssemblyImage, "DAGGer.Entity:OnTriggerBegin(single)");
			OnTriggerEndMethod = GetMethod(s_CoreAssemblyImage, "DAGGer.Entity:OnTriggerEnd(single)");
			OnCollision2DBeginMethod = GetMethod(s_CoreAssemblyImage, "DAGGer.Entity:OnCollision2DBegin(single)");
			OnCollision2DEndMethod = GetMethod(s_CoreAssemblyImage, "DAGGer.Entity:OnCollision2DEnd(single)");
		}
	};

	MonoObject* EntityInstance::GetInstance()
	{
		Dr_CORE_ASSERT(Handle, "Entity has not been instantiated!");
	...
	MonoAssembly* LoadAssemblyFromFile(const char* filepath)
	{
		...
		HANDLE file = CreateFileA(filepath, FILE_ALL_ACCESS, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	...
	static void InitMono()
	{
		...
		auto domain = mono_jit_init("DAGGer");

		char* name = (char*)"DAGGerRuntime";
	...
	void ScriptEngine::LoadDAGGerRuntimeAssembly(const std::string& path)
	{
		MonoDomain* domain = nullptr;
		bool cleanup = false;
		if (s_MonoDomain)
		{
			domain = mono_domain_create_appdomain("DAGGer Runtime", nullptr);
			mono_domain_set(domain, false);
			
			cleanup = true;
		}

		s_CoreAssembly = LoadAssembly("assets/scripts/DAGGer-ScriptCore.dll");
	...
	{
		LoadDAGGerRuntimeAssembly(path);
	...
	void ScriptEngine::Init(const std::string& assemblyPath)
	{
		...
		LoadDAGGerRuntimeAssembly(s_AssemblyPath);
	}
	...
	{
		Dr_CORE_ASSERT(s_EntityInstanceMap.find(dst) != s_EntityInstanceMap.end());
		Dr_CORE_ASSERT(s_EntityInstanceMap.find(src) != s_EntityInstanceMap.end());
		...
		for (auto& [entityID, entityInstanceData] : srcEntityMap)
		{
			for (auto& [moduleName, srcFieldMap] : srcEntityMap[entityID].ModuleFieldMap)
			{
				...
				{
					Dr_CORE_ASSERT(dstModuleFieldMap.find(moduleName) != dstModuleFieldMap.end());
					auto& fieldMap = dstModuleFieldMap.at(moduleName);
					Dr_CORE_ASSERT(fieldMap.find(fieldName) != fieldMap.end());
	...
	{
		Dr_CORE_ASSERT(s_EntityInstanceMap.find(sceneID) != s_EntityInstanceMap.end());
		auto& entityMap = s_EntityInstanceMap.at(sceneID);
		Dr_CORE_ASSERT(entityMap.find(entityID) != entityMap.end());
	...
	static FieldType GetDAGGerFieldType(MonoType* monoType)
	{
		...
		switch (type)
		{
			...
			case MONO_TYPE_VALUETYPE:
			{
				...
				if (strcmp(name, "DAGGer.Vector2") == 0) return FieldType::Vec2;
				if (strcmp(name, "DAGGer.Vector3") == 0) return FieldType::Vec3;
				if (strcmp(name, "DAGGer.Vector4") == 0) return FieldType::Vec4;
			}
		}
	...
	void ScriptEngine::InitScriptEntity(Entity entity)
	{
		...
		{
			Dr_CORE_ERROR("Entity references non-existent script module '{0}'", moduleName);
			return;
		}
		...
		// Retrieve public fields (TODO: cache these fields if the module is used more than once)
		{
			...
			while ((iter = mono_class_get_fields(scriptClass.Class, &ptr)) != NULL)
			{
				...
				FieldType DAGGerFieldType = GetDAGGerFieldType(fieldType);
				...
				{
					PublicField field = { name, DAGGerFieldType };
	...
	void ScriptEngine::InstantiateEntityClass(Entity entity)
	{
		...
		EntityInstance& entityInstance = entityInstanceData.Instance;
		Dr_CORE_ASSERT(entityInstance.ScriptClass);
	...
	{
		Dr_CORE_ASSERT(s_EntityInstanceMap.find(sceneID) != s_EntityInstanceMap.end(), "Invalid scene ID!");
		auto& entityIDMap = s_EntityInstanceMap.at(sceneID);
		Dr_CORE_ASSERT(entityIDMap.find(entityID) != entityIDMap.end(), "Invalid entity ID!");
	...
	static uint32_t GetFieldSize(FieldType type)
	{
		...
		Dr_CORE_ASSERT(false, "Unknown field type!");
		return 0;
	}
	...
	{
		Dr_CORE_ASSERT(m_EntityInstance->GetInstance());
