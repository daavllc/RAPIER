...
namespace DAGGer
{
	static GLenum ShaderTypeFromString(const std::string& type)
	{
		...
		Dr_CORE_ASSERT(false, "Unkown shader type!");
		return 0;
	}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			Dr_CORE_ASSERT(ShaderTypeFromString(type), "Invalid shader type specified");
	...
	void OpenGLShader::Compile(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		Dr_CORE_ASSERT(shaderSources.size() <= 2, "Only support 2 shaders (for now)");
		...
		for (auto& kv : shaderSources)
		{
			...
			if (isCompiled == GL_FALSE)
			{
				...
				Dr_CORE_ERROR("{0}", infoLog.data());
				Dr_CORE_ASSERT(false, "Shader compilation failure!");
				return;
			}
			glAttachShader(program, shader);	//	Attach shaders
		...
		if (isLinked == GL_FALSE)
		{
			...
			Dr_CORE_ERROR("{0}", infoLog.data());
			Dr_CORE_ASSERT(false, "Shader link failure!");
			return;
		}
