...
namespace DAGGer
{
	...
	namespace Utils
	{
		...
		static GLenum DAGGerFBTextureFormatToGL(FramebufferTextureFormat format)
		{
			switch (format)
			{
			...
			Dr_CORE_ASSERT(false, "Unkown DAGGer FRamebuffer Texture format!")
			return 0;
		}
	...
	void OpenGLFrameBuffer::Invalidate()
	{
		...
		{
			Dr_CORE_ASSERT(m_ColorAttachment.size() <= 4);
		...
		Dr_CORE_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "Framebuffer is incomplete!");
	...
	void OpenGLFrameBuffer::Resize(uint32_t width, uint32_t height)
	{
		...
		{
			Dr_CORE_WARN("Attempted to resize framebuffer to {0}, {1}", width, height);
			return;
		}
	...
	int OpenGLFrameBuffer::ReadPixel(uint32_t attachmentIndex, int x, int y)
	{
		...
		Dr_CORE_ASSERT(attachmentIndex < m_ColorAttachments.size(), "Invalid AttachmentIndex!");
	...
	void OpenGLFrameBuffer::ClearAttachment(uint32_t attachmentIndex, int value)
	{
		...
		glClearTexImage(m_ColorAttachments[attachmentIndex], 0, 
			Utils::DAGGerFBTextureFormatToGL(spec.TextureFormat), GL_INT, &value);
	}
