...
namespace RAPIER
{
	...
	namespace Utils
	{
		...
		static GLenum RAPIERFBTextureFormatToGL(FramebufferTextureFormat format)
		{
			switch (format)
			{
			...
			RP_CORE_ASSERT(false, "Unkown RAPIER FRamebuffer Texture format!")
			return 0;
		}
	...
	void OpenGLFrameBuffer::Invalidate()
	{
		...
		{
			RP_CORE_ASSERT(m_ColorAttachment.size() <= 4);
	...
	void OpenGLFrameBuffer::ClearAttachment(uint32_t attachmentIndex, int value)
	{
		...
		glClearTexImage(m_ColorAttachments[attachmentIndex], 0, 
			Utils::RAPIERFBTextureFormatToGL(spec.TextureFormat), GL_INT, &value);
	}
